...
namespace chipotto
{
	...
	OpcodeStatus Emulator::Opcode8(const uint16_t opcode)
	{
		...
		else if ((opcode & 0xF) == 0x6)
		{
			uint8_t registerX_index = (opcode >> 8) & 0xF;
			uint8_t registerY_index = (opcode >> 4) & 0xF;
			if (Registers[registerX_index] % 2 == 1) Registers[0xF] = 1;
			else Registers[0xF] = 0;
			//int result = static_cast<int>(Registers[registerX_index]) - Registers[registerY_index];
	...
	{
		uint8_t register_x = (opcode >> 8) & 0xF;
		uint8_t register_y = (opcode >> 4) & 0xF;
		std::cout << "SNE V" << (int)register_x << ", V" << (int)register_y;
	...
	uint8_t Emulator::GetSP() {
		return SP;
	}
	uint8_t Emulator::GetRegister(uint8_t value)
	{
...
//	if (emulator.IsValid())
//	{
//		emulator.LoadFromFile("C:\\Users\\alev4\\source\\repos\\CHIP-8\\CHIP-8\\TICTAC");
//		while (true)
//		{
//			if (!emulator.Tick())
//			{
